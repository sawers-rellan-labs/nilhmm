# call_gt(): closed-form per-site genotype caller with a swappable prior. The HWE
# prior is the het-excess control for the coverage-degradation study -- assert the
# decision rule exactly (worked-out log-posterior math), not by eyeball.
...
test_that("single ALT read: flat is het-blind (hom-ALT), HWE calls HET (het excess)", {
  ...
  expect_equal(call_gt(0, 1, prior = "flat"), 2L)
  # HWE with moderate p=0.3: P1 = log(.5)+log(2*.3*.7) beats P2 = log(.99)+2log(.3).
  expect_equal(call_gt(0, 1, prior = "hwe", af = 0.3), 1L)
  ...
  expect_equal(call_gt(0, 1, prior = "hwe", af = 0.06), 1L)
})

test_that("single REF read: both priors call REF (hom-REF)", {
  # (n_ref=1, n_alt=0): flat -> 0 (L0 > L1 > L2).
  expect_equal(call_gt(1, 0, prior = "flat"), 0L)
  # HWE p=0.3: P0 = log(.99)+2log(.7) is the max -> 0.
  expect_equal(call_gt(1, 0, prior = "hwe", af = 0.3), 0L)
})
...
test_that("high depth: the data dominates any prior", {
  ...
  for (pr in list(list(prior = "flat"),
                  ...
                  list(prior = c(0.98, 0.01, 0.01)))) {
    expect_equal(do.call(call_gt, c(list(0, 10), pr)), 2L)
    expect_equal(do.call(call_gt, c(list(10, 0), pr)), 0L)
  }
  # balanced high depth -> HET under any reasonable prior.
  expect_equal(call_gt(8, 8, prior = "hwe", af = 0.3), 1L)
})
...
test_that("numeric prior: design prior resists the low-depth het flip", {
  ...
  expect_equal(call_gt(0, 1, prior = c(2, 2, 2)), call_gt(0, 1, prior = "flat"))
})

test_that("zero depth -> NA (missing), preserved in matrix shape", {
  expect_true(is.na(call_gt(0, 0, prior = "flat")))
  expect_true(is.na(call_gt(0, 0, prior = "hwe", af = 0.2)))
  R <- matrix(c(0, 5, 0, 1), 2, 2); A <- matrix(c(0, 0, 3, 0), 2, 2)
  out <- call_gt(R, A, prior = "flat")
...
test_that("vectorized matrix input returns the right shape; vector in -> vector out", {
  ...
  af <- runif(M, 0.05, 0.5)
  out <- call_gt(R, A, prior = "hwe", af = af)
  ...
  vout <- call_gt(R[, 1], A[, 1], prior = "hwe", af = af)
...
test_that("bad inputs are rejected", {
  expect_error(call_gt(c(0, 1), c(1, 0), prior = "hwe", af = 0.3), "length")
  expect_error(call_gt(1, 0, prior = "flat", error = 0.6), "error")
  expect_error(call_gt(1, 0, prior = c(0.5, 0.5)), "length 3")
  expect_error(call_gt(1, 0, prior = c(0, 0, 0)), "all zero")
  expect_error(call_gt(matrix(0, 2, 2), matrix(0, 2, 3), prior = "flat"), "shape")
})
...
test_that("af = NULL estimates p per marker from the reads", {
  ...
  out <- call_gt(R, A, prior = "hwe", af = NULL)
...
test_that("return = 'post' gives normalized posteriors summing to 1; NA at zero depth", {
  post <- call_gt(c(0, 0, 2), c(1, 0, 8), prior = "hwe", af = c(0.3, 0.3, 0.3),
                  return = "post")
  ...
  d <- call_gt(c(0, 1), c(1, 0), prior = "hwe", af = c(0.3, 0.3), return = "dosage")
