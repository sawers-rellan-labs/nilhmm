# ATLAS caller (R/atlas.R): GOOGA competitive-alignment ancestry. Hard fraction
# call (recurrent = n_ref, donor = n_alt; ambiguous excluded upstream) -> gt
# (categorical confusion) emission HMM.
...
test_that("call_ancestry(caller='atlas') recovers a donor block; control is all-REF", {
  set.seed(1)
  ...
  calls <- call_ancestry(data, caller = "atlas", design = "BC2S3", source = "atlas")
  ...
  expect_true(any(alt$start_bp <= 70e6 & alt$end_bp >= 40e6))   # donor block detected as ALT

  ctrl <- data.frame(name = "B73", chr = 1L, pos = as.integer(pos), n_ref = 20L, n_alt = 0L)
  cc <- call_ancestry(ctrl, caller = "atlas", design = "BC2S3")
  expect_true(all(cc$state == 0L))                              # pure recurrent -> all REF
})

test_that("atlas gates on min informative reads (low-depth genes -> no donor)", {
  ...
  calls <- call_ancestry(data, caller = "atlas", design = "BC2S3")
  expect_true(all(calls$state == 0L))                          # all missing -> smoothed to REF
})
