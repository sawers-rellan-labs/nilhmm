...
and `atlas` = gt + **rigidity** is this work’s transcript caller.
`ml`/`hwemap` are the no-HMM per-site baselines (backed by
[`call_gt()`](https://sawers-rellan-labs.github.io/nilhmm/reference/call_gt.md)):
`ml` = flat-prior maximum likelihood, `hwemap` = HWE-prior MAP (the
het-excess reference). Neither involves
[`interpolate_genotype()`](https://sawers-rellan-labs.github.io/nilhmm/reference/interpolate_genotype.md),
which is a separate deterministic densifier, not a caller.
