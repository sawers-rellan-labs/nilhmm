...
- `nnil` — the categorical caller on hard genotype calls (Holland’s
  error model: `germ`, `gert`, `p`, `mr`, `nir`); a `g` column is used
  directly, read counts are first hard-called (1/3–2/3 cutoffs).
  Geometric duration, so the self-transition is the smoother.
