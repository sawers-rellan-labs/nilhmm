...
.binhmm_states <- function(data, bin_size, cluster_method, priors,
  ...
  # Drop truly-empty bins (no informative markers): left in, a bin defaults to
  # alt_freq = 0 and would be silently called REF — a confident call from no data.
  # This is an internal safety guard, NOT a user knob (binhmm has no read-depth
  # parameter; a future per-bin frequency gate would be a separate `min_freq`).
  bins <- bins[bins$ninf >= 1L, , drop = FALSE]
  if (!nrow(bins)) stop("binhmm: no bins with any informative markers")
