...
#> 1 B1_L1     B1       A      B1          0.75          0.25 0.9375
#> 2 B1_L2     B1       A      B1          0.75          0.25 0.9375
```
...
FSFHap pools *each* family independently, so a realistic run has more
than one — each a distinct donor crossed onto the shared recurrent
parent **A**. We simulate three **BC1S4** families with
[**simcross**](https://cran.r-project.org/package=simcross), donors
**B1**, **B2**, **B3** (the TeoNAM design: one backcross to A, then four
selfings — `F = 0.9375`, so `phet = 0.03125`). Each family needs enough
segregating markers to phase its donor and clear FSFHap’s coverage
floor, so we use 20 lines × 500 markers. simcross gives real
recombination breakpoints; the donor blocks and residual heterozygosity
fall out of the crossing scheme.
...
Each **family × chromosome** is an independent unit — here three
families on one chromosome, so three units — and `fsfhap` fans them out
across cores with `threads` (bit-identical to serial):
...
On a real full-genome cohort this is a large win. On real TeoNAM (all
five families, 51,004 markers, 50 family × chromosome units) the port
